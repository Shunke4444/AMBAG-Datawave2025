  ...
  return (
    <main className="flex items-center justify-between pt-8 px-5 bg-primary text-secondary">
      <section className="flex items-center space-x-3">
        <img src={userIcon} className="w-12 h-12 contain-content" />
        <h1 className="text-lg font-medium">Hello, {userName}</h1>
      </section>
      <aside className="flex items-center space-x-4">
        <div className="relative">
          <button onClick={handleChatClick} className="w-10 h-10 text-accent rounded-lg flex items-center justify-center hover:bg-secondary/10 transition-colors cursor-pointer">
            <img src={bot} alt="Chatbot Icon" className="w-7 h-7 filter-accent" style={{filter: 'brightness(0) saturate(100%) invert(100%)'}} />
          </button>
        </div>
